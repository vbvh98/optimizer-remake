...
const Result = ({ history, tableData, profileLength, ...rest }) => {
    ...
    React.useEffect(() => {
        // if (localStorage.hasOwnProperty("result")) setResult(JSON.parse(localStorage.getItem("result")));
        // else {
        const solution = greedy_solution(tableData, profileLength).map(e => JSON.stringify({ items: e.items, unused: e.space_remaning }));
        ...
        // localStorage.setItem("result", JSON.stringify(final.sort((a, b) => b.count - a.count)));
        // }
                                            ...
                                            <tbody>
                                                <tr>
                                                    {profile.items.map((i, j) => (
                                                        <td key={j} style={{ borderLeft: "1px solid black", width: `${(i.length * 100) / profileLength}%` }}>
